 ...
 *
 * @package Datadev_Jadlog/Classes/Orders
 * @since   1.0.0
 * @version 1.0.0
 */
...
class Datadev_Jadlog_Orders {
    ...
    public function shipping_method_delivery_forecast($name, $order) {
        ...
        foreach ($order->get_shipping_methods() as $shipping_method) {
            ...
            if ($total) {
                ...
                $names[] = sprintf(_n('%1$s (delivery within %2$d working day after paid)', '%1$s (delivery within %2$d working days after paid)', $total, 'datadev-jadlog-for-woocommerce'), $shipping_method->get_name(), $total);
            } else {
    ...
    public function item_display_delivery_forecast($display_key, $meta) {
        return '_delivery_forecast' === $meta->key ? __('Delivery forecast', 'datadev-jadlog-for-woocommerce') : $display_key;
    }

}

new Datadev_Jadlog_Orders();
